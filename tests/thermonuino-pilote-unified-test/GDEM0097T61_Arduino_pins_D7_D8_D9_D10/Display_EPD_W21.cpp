...
void EPD_WhiteScreen_ALL_4Gray(const unsigned char *datas)
{
   ...
   {               
		tempOriginal= In2bytes_Out1byte_RAM1(pgm_read_byte(&datas[i]),pgm_read_byte(&datas[i+1]));
		 EPD_W21_WriteDATA(~tempOriginal); 
   } 
	 EPD_W21_WriteCMD(0x26);   //write RAM for black(0)/white (1)
   for(i=0;i<EPD_ARRAY*2;i+=2)
   {               
		tempOriginal= In2bytes_Out1byte_RAM2(pgm_read_byte(&datas[i]),pgm_read_byte(&datas[i+1]));
		 EPD_W21_WriteDATA(~tempOriginal); 
   }
   EPD_Update_Fast();	 	 
}
