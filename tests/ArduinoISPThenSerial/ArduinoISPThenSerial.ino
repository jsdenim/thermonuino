...
//NOTES : 
//Il faut fermer le moniteur de console pour faire un nouvel upload.
//Il faut peut-être appuyer sur le bouton reset avant de faire un upload.
...
// Serial relay config (target UART over MOSI/MISO after programming)
#define RELAY_BAUDRATE 9600
...
static SoftwareSerial relaySerial(ARDUINOISP_PIN_MISO, ARDUINOISP_PIN_MOSI);
...
void start_relay() {
  // Release SPI and switch MOSI/MISO to software serial for target UART.
  pinMode(ARDUINOISP_PIN_SCK, INPUT);
  pinMode(ARDUINOISP_PIN_MISO, INPUT);
  pinMode(ARDUINOISP_PIN_MOSI, OUTPUT);
  relaySerial.begin(RELAY_BAUDRATE);
